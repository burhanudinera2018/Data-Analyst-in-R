...
knitr::opts_knit$set(root.dir = "./Users/burhanudin/Study_burhanudin_6/R/Code-R/Data_Analyst_in_R/")
getwd()
```
```{r}
telecomDataframe <- read.csv(file = "data/WA_Fn-UseC_-Telco-Customer-Churn.csv")
```
...
```{r}
telecomModel <- glm(as.factor(Churn) ~ .,family=binomial(link="logit"),data=trainData)
print(summary(telecomModel))
```

<div class="alert alert-block alert-warning">Error in eval(family$initialize) : y values must be 0 <= y <= 1 ,

This follow error can be solved by using as.factor(Churn) the following error is cleared</div>
